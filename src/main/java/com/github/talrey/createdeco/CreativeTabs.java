...
public class CreativeTabs {
  ...
  private static final DeferredHolder<CreativeModeTab, CreativeModeTab> PROPS = TABS.register(
          CreativeTabs.PROPS_KEY, () -> group(
                  CreativeTabs.PROPS_KEY, "Create Deco Props", () -> BlockRegistry.GREEN_CAGE_LAMPS.get("Brass").asStack()
          )
  );

  private static final DeferredHolder<CreativeModeTab, CreativeModeTab> BRICKS = TABS.register(
          CreativeTabs.BRICKS_KEY, () -> group(
                  CreativeTabs.BRICKS_KEY, "Create Deco Bricks", () -> BlockRegistry.BRICKS.get(DyeColor.LIGHT_BLUE).get("blue_bricks").asStack()
          )
  );
  ...
  private static CreativeModeTab group (String name, String fallback, Supplier<ItemStack> item) {
    CreativeModeTab tab = CreativeModeTab.builder().icon(item)
            .title(Component.translatableWithFallback(
                    "itemGroup." + CreateDecoMod.MOD_ID + "." + name, fallback
            ))
            .build();
    return tab;
  }
